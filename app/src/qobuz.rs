//! Qobuz client for what the app does at runtime: browsing, stream URLs, and
//! writing a path out as a playlist.
//!
...
/// Qobuz format ids, same set the Python side uses.
pub const FORMAT_MP3_320: u32 = 5;
pub const FORMAT_FLAC_CD: u32 = 6;
pub const FORMAT_FLAC_HIRES: u32 = 7;

/// Requests per second, and how many may be spent at once. Same budget the
/// Python crawler uses, since both talk to one account.
...
impl Credentials {
    /// Same lookup order as the Python side: the qobuz_stream .env supplies
    /// app_id/app_secret, the repo .env supplies the login, the environment wins.
...
impl QobuzClient {
    ...
    /// Wait for a request slot. Mirrors `_RateLimiter` on the Python side.
